...
export default function StaffPage() {
  const { staff, inviteStaff, deactivateStaff, live, loading, backendError } = useAdmin();
    ...
    <div className="space-y-4">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Invite staff">
        <div className="space-y-3">
          ...
          <p className="text-[0.78rem] text-ink-secondary">Creates a hospital-admin login for this hospital{live ? "" : " (local demo)"}.</p>
          ...
          <Button className="w-full" disabled={!email.trim() || password.length < 8 || busy} onClick={() => void sendInvite()}>
            {busy ? "Sending…" : live ? "Send invite" : "Send invite (mock)"}
          </Button>
        </div>
      </Modal>
    </div>
