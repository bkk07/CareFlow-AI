...
const PLATFORM_NAV = [
  { to: "/platform", label: "Platform Overview", icon: LayoutDashboard, end: true, group: "" },
  { to: "/platform/applications", label: "Hospital Applications", icon: Building2, end: false, group: "" },
  { to: "/platform/hospitals", label: "Hospitals", icon: Globe, end: false, group: "Directory" },
  { to: "/platform/doctors", label: "Doctors", icon: Stethoscope, end: false, group: "Directory" },
  { to: "/platform/patients", label: "Patients", icon: Users, end: false, group: "Directory" },
  { to: "/platform/appointments", label: "Appointments", icon: CalendarDays, end: false, group: "Directory" },
  { to: "/platform/ai", label: "AI Activity", icon: Activity, end: false, group: "Insights" },
  { to: "/platform/integrations", label: "Integrations", icon: Settings, end: false, group: "Insights" },
  { to: "/platform/workflows", label: "Workflows", icon: Workflow, end: false, group: "Insights" },
  { to: "/platform/analytics", label: "Analytics", icon: Activity, end: false, group: "Insights" },
  { to: "/platform/audit", label: "Audit Logs", icon: ClipboardList, end: false, group: "" },
  { to: "/platform/health", label: "Operational Health", icon: HeartPulse, end: false, group: "" },
];

function NavList({ items }: { items: typeof PLATFORM_NAV }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-[252px] flex-col bg-white border-r border-border px-3.5 py-4 z-30" aria-label="Primary">
        <Link to="/platform" className="flex items-center gap-2.5 px-1" aria-label="CareFlow AI home">
          ...
          <span className="leading-none">
            ...
            <span className="block text-[0.64rem] font-bold uppercase tracking-widest text-ink-faint mt-0.5">Platform Console</span>
          </span>
        </Link>
        <div className="mt-3 px-1">
          <p className="text-[0.68rem] font-bold uppercase tracking-widest text-ink-faint">Viewing as</p>
          <p className="text-[0.85rem] font-bold text-navy mt-0.5">Platform Admin</p>
        </div>
        <div className="flex-1 overflow-y-auto mt-2 pb-2"><NavList items={PLATFORM_NAV} /></div>
      ...
      <header className="md:pl-[252px] sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-border">
        <div className="max-w-shell mx-auto px-4 sm:px-6 h-[60px] flex items-center gap-3">
          <div className="min-w-0">
            <p className="font-bold text-navy text-[0.9rem] truncate">CareFlow Platform</p>
            <p className="text-[0.72rem] text-ink-secondary leading-none">{live && user ? user.email : "Platform Administrator"}</p>
          </div>
          ...
          <Avatar name={user?.email ?? "Platform Administrator"} size="sm" />
        </div>
      </header>
