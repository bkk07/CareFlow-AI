...
import { EmptyState, MetricCard, StatusBadge } from "../../components/common/ui";
import { Drawer, ResponsiveTable } from "../../components/common/Modal";
import type { AuditEvent } from "../../types";

export function PlatformIntegrationsPage() {
  const { integrations, loading, backendError } = useAdmin();
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && <p className="text-[0.83rem] text-ink-secondary">Loading integrations…</p>}
      {backendError && (
      ...
      {integrations.length === 0 && !loading && (
        <div className="card-base"><EmptyState title="No integrations yet" body="No hospitals registered yet. Integrations reported by the backend will appear here." /></div>
      )}
...
export function PlatformWorkflowsPage() {
  const { workflows, loading, backendError } = useAdmin();
  return (
    <div className="space-y-4">
      ...
      {loading && <p className="text-[0.83rem] text-ink-secondary">Loading workflows…</p>}
      {backendError && (
        <p role="alert" className="text-[0.83rem] font-semibold text-danger bg-danger-soft border border-danger/20 rounded-control px-3 py-2.5">{backendError}</p>
      )}
      {workflows.length === 0 && !loading ? (
        <div className="card-base"><EmptyState title="No workflows yet" body="No workflow executions recorded yet." /></div>
      ) : workflows.length > 0 ? (
...
export function PlatformAnalyticsPage() {
  const { analytics, loading, backendError } = useAdmin();
  if (loading && !analytics) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[0.83rem] text-ink-secondary">Loading analytics…</p>
      </div>
    );
  }
  ...
  if (!analytics) {
    return (
      <div className="space-y-4">
        <div><h1 className="page-title">Platform Analytics</h1><p className="page-sub mt-1">Network-level trends.</p></div>
        <LiveBanner text="Live network analytics" />
        <div className="card-base"><EmptyState title="No analytics yet" body="Network analytics reported by the backend will appear here." /></div>
      </div>
    );
  }
...
export function AuditPage() {
  const { audit, loading, backendError } = useAdmin();
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && <p className="text-[0.83rem] text-ink-secondary">Loading audit events…</p>}
      {backendError && (
        <p role="alert" className="text-[0.83rem] font-semibold text-danger bg-danger-soft border border-danger/20 rounded-control px-3 py-2.5">{backendError}</p>
      )}
      ...
      {visible.length === 0 && !loading ? (
        <div className="card-base"><EmptyState title="No audit events yet" body="Audit events recorded by the backend will appear here." /></div>
      ) : visible.length > 0 ? (
...
export function OperationalHealthPage() {
  const { metrics, aiEvaluation, loading, backendError } = useAdmin();
  if (loading && !metrics) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[0.83rem] text-ink-secondary">Loading operational health…</p>
      </div>
    );
  }
  if (backendError && !metrics) {
    return (
      <div className="space-y-4">
        <div><h1 className="page-title">Operational Health</h1><p className="page-sub mt-1">System status at a glance.</p></div>
        <LiveBanner text="Live observability metrics" />
  ...
  if (!metrics) {
    return (
      <div className="space-y-4">
        <div><h1 className="page-title">Operational Health</h1><p className="page-sub mt-1">System status at a glance.</p></div>
        <LiveBanner text="Live observability metrics" />
        <div className="card-base"><EmptyState title="No health data yet" body="Observability metrics reported by the backend will appear here." /></div>
      </div>
    );
  }
