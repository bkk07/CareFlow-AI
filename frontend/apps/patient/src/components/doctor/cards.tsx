    ...
    >
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {(doctor.experienceYears > 0 || doctor.languages.length > 0) && (
            <p className="text-[0.8rem] text-ink-secondary mt-1">
              {[doctor.experienceYears > 0 ? `${doctor.experienceYears} yrs experience` : null, doctor.languages.length > 0 ? doctor.languages.join(", ") : null]
                .filter(Boolean)
                .join(" · ")}
            </p>
          )}
