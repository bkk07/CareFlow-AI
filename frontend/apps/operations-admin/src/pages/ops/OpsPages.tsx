...
import { Button, EmptyState, MetricCard, StatusBadge, TableSkeleton } from "../../components/common/ui";
import { Drawer, Modal, ResponsiveTable } from "../../components/common/Modal";
import type { Operation } from "../../types";

export function LiveBanner({ text }: { text: string }) {
  const { live, loading, backendError, refreshAll } = useAdmin();
  if (!live) return null;
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => void refreshAll().catch(() => undefined)} className="text-[0.8rem] font-bold text-healthcare hover:underline">Refresh</button>
      </div>
      {backendError && (
...
export function OpsOverviewPage() {
  const { operations, reconciliations, escalations, loading, backendError } = useAdmin();
  ...
  return (
    <div className="space-y-5">
      ...
      ) : backendError && operations.length === 0 ? (
        <div className="card-base"><EmptyState title="Could not load operations" body={backendError} /></div>
      ) : operations.length === 0 ? (
        <div className="card-base"><EmptyState title="No operations yet" body="Live operations will appear here once the backend records vendor calls." /></div>
      ) : (
...
export function FailedOpsPage() {
  const { operations, retryOperation, loading, backendError } = useAdmin();
  const [selected, setSelected] = useState<Operation | null>(null);
  const { error, run } = useOpError();
  ...
  return (
    <div className="space-y-4">
      ...
      {(error ?? backendError) && failed.length > 0 && <p role="alert" className="text-[0.83rem] font-semibold text-danger">{error ?? backendError}</p>}
      {loading && operations.length === 0 ? (
        <TableSkeleton rows={5} cols={7} />
      ) : failed.length === 0 ? (
        <div className="card-base"><EmptyState title="No failed operations" body={backendError ?? "All integration operations are currently healthy."} /></div>
      ) : (
...
export function UnknownOutcomesPage() {
  const { operations, verifyOperation, loading, backendError } = useAdmin();
  const [selected, setSelected] = useState<Operation | null>(null);
  const [outcome, setOutcome] = useState<string | null>(null);
  const { error, run } = useOpError();
  ...
  return (
    <div className="space-y-4">
      ...
      {(error ?? backendError) && unknown.length > 0 && <p role="alert" className="text-[0.83rem] font-semibold text-danger">{error ?? backendError}</p>}
      {outcome && <p className="text-[0.83rem] font-semibold text-success">Verification outcome: {outcome}</p>}

      {loading && operations.length === 0 ? (
        <TableSkeleton rows={4} cols={6} />
      ) : unknown.length === 0 ? (
        <div className="card-base"><EmptyState title="No unknown outcomes" body={backendError ?? "All operations have a determined final state."} /></div>
      ) : (
...
export function ReconciliationPage() {
  const { reconciliations, resolveReconciliation, retryReconciliation, loading, backendError } = useAdmin();
  ...
  const openCases = reconciliations.filter((r) => r.resolution !== "resolved");
  ...
  return (
    <div className="space-y-4">
      ...
      {(error ?? backendError) && openCases.length > 0 && <p role="alert" className="text-[0.83rem] font-semibold text-danger">{error ?? backendError}</p>}
      {loading && reconciliations.length === 0 ? (
        <TableSkeleton rows={4} cols={6} />
      ) : openCases.length === 0 ? (
        <div className="card-base"><EmptyState title="All integration operations are currently reconciled." body={backendError ?? "New mismatches will open a case here."} /></div>
      ) : (
...
export function EscalationsPage() {
  const { escalations, assignEscalation, resolveEscalation, loading, backendError } = useAdmin();
  const { error, run } = useOpError();
  ...
  return (
    <div className="space-y-4">
      ...
      {(error ?? backendError) && open.length > 0 && <p role="alert" className="text-[0.83rem] font-semibold text-danger">{error ?? backendError}</p>}
      {loading && escalations.length === 0 ? (
        <TableSkeleton rows={3} cols={3} />
      ) : open.length === 0 ? (
        <div className="card-base"><EmptyState title="No human escalations" body={backendError ?? "Cases needing operator judgment will queue here."} /></div>
      ) : (
...
export function RetryQueuePage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {(error ?? backendError) && queue.length > 0 && <p role="alert" className="text-[0.83rem] font-semibold text-danger">{error ?? backendError}</p>}
      {loading && operations.length === 0 ? (
        <TableSkeleton rows={4} cols={5} />
      ) : queue.length === 0 ? (
        <div className="card-base"><EmptyState title="Retry queue is empty" body={backendError ?? "Failed operations scheduled for retry will appear here."} /></div>
      ) : (
...
export function RecoveryHistoryPage() {
  const { reconciliations, loading, backendError } = useAdmin();
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && reconciliations.length === 0 ? (
        ...
        <div className="card-base"><EmptyState title="No recoveries yet" body={backendError ?? "Resolved reconciliation cases will be recorded here."} /></div>
      ) : (
        <div className="space-y-2.5">
          {recovered.map((r) => (
